...
import { cloneListProducts, NEO_BILLING_OPTIONS, type NeoBillingOption } from "@/lib/cart-clone";
...
export async function POST(
  ...
  const requestedTargetRegion = getTrimmedString(body?.targetRegion);
  const requestedTargetBillingMode = getTrimmedString(body?.targetBillingMode);
  ...
  const access = getProjectAccessForUser(apiKeyUser.userId, projectId);
  ...
  const clonedLists = await Promise.all(
    sourceLists.map(async (sourceList) => {
      ...
      const cloneInputProducts = listProducts;
  ...
  db.transaction(() => {
    ...
    for (const list of clonedLists) {
      createListRecord({ id: list.id, projectId: newProjectId, userId: apiKeyUser.userId, name: list.name, now });
